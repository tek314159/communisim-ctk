...
function person(firstname, surname, age, gender) {
    ...
    this.chooseActivity = function() {
        ...
        var ch = weightedChooser(regActivities, ac);
        //console.log(ch);
        return ch;
    }
...
/* function dayEnd() {
    for (var i = 0; i < liveNPC.length; i++) {
        ...
        $("#npcactivity" + liveNPC[i]).html(npc[liveNPC[i]].doActivity(npc[liveNPC[i]].chooseActivity()));
    }
    counter++;
    resolveEvents();
    evolveRelationships();
...
var petDash = (
    ...
    '<button type="button" class="btn btn-primary" id="rewardbutton" onClick="rewardPet()">Reward Pet</button> ' + 
    '<button type="button" class="btn btn-primary" id="punishbutton" onClick="punishPet()">Punish Pet</button></p>'

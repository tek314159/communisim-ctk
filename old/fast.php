        ...
        <div class="row">
            <div class="col-md">
                ...
                <div id="devdash">
                    ...
                    <div id="footpanel">
                    <button type="button" class="btn btn-secondary" id="popbutton">Clear Popovers</button>
                    </div>
            </div>
...
function dLc() {
    ...
    girlNames.push(
        "Emi", "Sun Lei", "Melody", "Jingjing", "Donna", "Kink", "Chang Ying", "Yvonne", "Cyndi", "Elana"
    );
    boyNames.push(
        "Tek"
    );
...
var specialEvents = [
    ...
    },
    {
        ...
        "effect": function() {
            socialNormConstant = 500;
            kissBackReq = -5000;
            textLog += (textLogPrefix() + "<strong>strange water</strong> started flowing from the taps.<br>");
            postEventQueue.push([counter+365,function(){socialNormConstant = 2000;kissBackReq = 150;textLog+=(textLogPrefix() + "<strong>water</strong> has returned to normal.<br>")}])
        },
        "flavortext":"Something happens!!"
    },
    {
...
var regActivities = [
    ...
    },
    {
        "title":"project",
        "active":1,
        "reqFunction": function() {if (randomInt(1,100) == 1) {return 1} return 0},
...
var kissReq = 1500; // min attraction to try to kiss
var kissBackReq = 750; // min attraction to return kiss
...
function whupEee(persona, personb) {
    if (persona.rTwo[npc.indexOf(personb)].attraction > kissReq && testSocialNorms(persona, personb) == 0 && randomInt(1,(12-persona.social)) == 1) {
...
function resolvePregnancies() {
    for (var i=0;i<npc.length;i++) {
        if ((counter - npc[i].pregnancyStart) > 270 && randomInt(1, 20) == 1 && npc[i].alive == 1) {
            ...
            var mother = npc[i];
            var fnum = (npc[i].pregnancyParent);
            var father = npc[fnum];
            ...
            npc[i].pregnancyParent = undefined;
            for (var l=0;l<10;l++) {
...
function fullCharInfoPanel(char) {
    ...
    for (var h=0;h<raray.length;h++) {
        ra += ("<li>" + raray[h].rName + " | Friendship: " + raray[h].friendship + " | Attraction: " + raray[h].attraction + " | Romance: " + raray[h].romance + " | Interactions: " + raray[h].interactions + " | Coitus: " + raray[h].coitus + "</li>")
    }
    var pnl = (
...
$("#popbutton").click(function() {$("[data-toggle='popover']").popover('hide');});
...
function updateMetaDash() {
    $("#metadash").html("Project Material: " + projectMaterial + "<br>");
}

...
function weightedChooser(array1, array2) {
    ...
    console.log(c + " is c; choice is " + choice);
    ...
    if (pleasure > 3) {
        if (persona.attractions[npc.indexOf(personb)] != undefined) {
            ...
            for (k=0;k<persona.myTypes.length;k++) {
                ...
                //console.log(persona.myTypes[k].typeTest(persona, personb));
            }
            persona.attractions[npc.indexOf(personb)] += bonus;
            if (persona.attractions[npc.indexOf(personb)] > 100) {
                if (randomInt(1,3) == 1) {
                    $("#textlog").append(persona.firstName + " reaches out to kiss " + personb.firstName + ". ");
                    if (personb.attractions[npc.indexOf(persona)] > 50) {
                        $("#textlog").append(personb.firstName + " kisses her back.<br>");
                    } else {
                        $("#textlog").append(personb.firstName + " pulls away.<br>");
                    }
                }
            }
        } else {
...
function meetPeopleTwo() {
    for (i=0;i<npc.length;i++) {
        ...
        /* 
            ...
            //console.log(p + " is p");
        while (npc.indexOf(u) == i) {
            var u = weightedChooser(npc, p);
        }
        console.log(npc[i]);
        console.log("that was npc i");
        interactWith(npc[i], u); */
    }
}
...
function gameSetup() {
    ...
    for (i=0;i<npc.length;i++) {
        $("#dashpanel").append(
            ...
            "<div class='col-4'>Beauty: "+npc[i].beauty+"<br>Athletics: "+npc[i].athletics+"<br>Intellect: "+npc[i].intellect+"<br>Social: "+npc[i].social+"<br></div>"+"</div>"
        );
    }
}
...
function updateDash() {
    for (i = 0; i < npc.length; i++) {
        ...
        var des = "";
        for (j = 0; j < npc.length; j++) {
            if (j != i && npc[i].rTwo[j] != undefined) {
                des += (npc[i].firstName + relationshipDescriptors(npc[i].rTwo[j].friendship) + npc[j].firstName + ". ");

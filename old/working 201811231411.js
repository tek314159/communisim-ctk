...
var regActivities = [
    ...
    },
    {
        "title":"explore",
        "active":0,
        "reqFunction": function(char) {if (this.active == 1 && char.ageInYears() > 15) {return 1} else {return 0}},
...
function masterLoop() { // this is a replacement for dayEnd
    ...
    if (((counter + 180) % 365) == 0) {
        if (birthRate[thisYear()] != undefined) {
            birthRate[thisYear()][1] = popCalc().f;
        } else {birthRate[thisYear()] = [0,popCalc().f,0]}
    }
    if (((counter) % 364) == 0 && birthRate[thisYear()][0] != 0) {
        birthRate[thisYear()][2] = (birthRate[thisYear()][0]/birthRate[thisYear()][1])
    }
}
...
function updateDash() {
    ...
    $("#fpop").text(popCalc().f);
    $("#mpop").text(popCalc().m);
    $("#temptext").html(`Year: ${thisYear()}, Day: ${parseYear(counter).day}`)
    var tPadder = 0;

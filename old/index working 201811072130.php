        ...
        <div class="row">
            <div class="col-md">
                ...
                <div id="devdash">
                    ...
                    <button type="button" class="btn btn-primary" id="testbutton2" data-toggle="modal" data-target="#exampleModal">Modal</button>
...
var specialEvents = [
    {
        ...
        "checkOccurrance": function() {console.log("this is an event")},
        "effect": function() {counter += 1},
...
var regActivities = [
    ...
    },
    {
        ...
        "flavortext":" is watching TV.",
        "enjoyFunction": function(char) {
            var hch = 5 - findDiff(char.intellect, 4);
            return hch;
        }
    },
    {
...
var gayWeightScale = [1,5,4,2,5,5,5,10,30,1];
...
function calcAttraction(persona, personb) {
    ...
    bonus += (persona.beauty - personb.beauty);
...
var kissReq = 100; // min attraction to try to kiss
var kissBackReq = 60; // min attraction to return kiss
...
function whupEee(persona, personb) {
    ...
    var wadj = Math.floor(persona.rTwo[npc.indexOf(personb)].attraction / 2500);
    if (w > 0 && wadj > 0) {
        ...
        if (w == 0) {console.log(persona.firstName + " is violating social norms.");}
    }
    if (persona.rTwo[npc.indexOf(personb)].attraction > kissReq && w == 0 && randomInt(1,(15-persona.social)) == 1) {
        ...
        if (personb.rTwo[npc.indexOf(persona)].attraction > kissBackReq) {
            $("#temptext").append(personb.firstName + " kisses " + persona.oPronoun() + " back.<br>");
            ...
            if (persona.rTwo[npc.indexOf(personb)].romance > 60) {
                ...
                if (persona.rTwo[npc.indexOf(personb)].commitment == 0 && sti == 0) {
                    ...
                    textLog += ("On day " + counter + ", " + persona.firstName + " promises to be loyal to " + personb.firstName + ".<br>");
                }
            }
            if (randomInt(1,5) == 1) {
                if (persona.rTwo[npc.indexOf(personb)].coitus == 0) {
                    console.log("first whup");
                    textLog += ("On day " + counter + ", " + persona.firstName + " (age " + persona.ageInYears() + ") and " + personb.firstName + " (age " + personb.ageInYears() + ")  slept together for the first time.<br>");
                }
                ...
                if (pregChances(persona, personb) == 1) {
                    if (persona.gender == 0) {
                        ...
                        textLog += ("On day " + counter + ", " + persona.firstName + " became pregnant.<br>");
                    } else {
                        personb.pregnant = 1;
                        personb.pregnancyStart = counter;
                        textLog += ("On day " + counter + ", " + personb.firstName + " became pregnant.<br>");
                    }
                }
            }
        } else {
...
function describeRelationship(persona, personb) {
    ...
    if (persona.relatives[npc.indexOf(personb)] != undefined) {
        ...
        if (persona.rTwo[npc.indexOf(personb)].attraction > 500 && persona.ageInYears() >= 6) {
    ...
    // lovers
        description += (personb.firstName + " is " + persona.firstName + "'s lover. ");
    } else {
    // friends
...
function resolvePregnancies() {
    for (var i=0;i<npc.length;i++) {
        if ((counter - npc[i].pregnancyStart) > 270 && randomInt(1, 20) == 1) {
            ...
            textLog += ("On day " + counter + ", " + npc[i].firstName + " (age " + npc[i].ageInYears() + ") gave birth to " + newb.firstName + ".<br>");
            for (var l=0;l<10;l++) {
...
function resolveDeaths() {
    for (var i=0;i<npc.length;i++) {
        if (npc[i].ageInYears() > 75 && npc[i].alive == 1 && randomInt(1, 20 * 365) == 1) {
            ...
            textLog += ("On day " + counter + ", " + npc[i].firstName + " died at age " + npc[i].ageInYears() + ".<br>");
            displayCharPanels();        
        }
    }
}
...
function createCharPanel(id) {
    var panel = (
        "<div class='row charcard' id='npc"+id+"'><div class='col-8'>Name: <a href='javascript:;' data-npcid='"+id+"' onclick='modalFunction(" + id + ")' class='charname' data-toggle='modal' data-target='#exampleModal'><span id='npcname" + id + "'><strong><a href='javascript:;'></strong></span></a> (<span id='npcgender" + id + "'></span>)<br>" +
            ...
            '<a href="javascript:;" data-toggle="popover" data-container="body" data-trigger="hover" data-offset="5" data-html="true" title="Relationships" data-content="No friends yet." id="npcrel' + id + '">Relationships</a></div>' +
...
function modalFunction(id) {
    ...
    $("#modalbody").html(allRelationships(npc[id]));
}

$("#testbutton").click(function() {printStats();});
$("#testbutton2").click(function() {printStats();});

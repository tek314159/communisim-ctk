        ...
        <div class="col-md">
            ...
            <div id="devdash">
                <p>
                    ...
                    <button type="button" class="btn btn-secondary" id="murderbutton">Kill</button>
...
var specialEvents = [
    ...
    },
    {
        ...
        "effect": function() {
            ...
            postEventQueue.push([counter+180,function(){socialNormConstant = orisoc;kissBackReq = orikbr;textLog.push([counter, "<strong>water</strong> has returned to normal."])},this.title])
        },
        "flavortext":"Something happens!!"
    },
    {
    ...
    },
    {
        ...
        "odds":300,
        "checkOccurrance": function() {
            if (popCalc().pop > 80 && randomInt(1,(this.odds - popCalc().pop)) == 1) {
                return 1;
            } else {
                return 0;
            }
...
function allRelationships(char) {
    var des = [];
    for (var j = 0; j < npc.length; j++) {
        if (char.rTwo[j] != undefined) {
            if (describeRelationship(char, npc[j]) != 0 && npc[j].alive == 1 && (findDiff(char.rTwo[j].friendship,0) > 500)) {
    ...
    for (var i=0;i<npc.length;i++) {
        for (var j=0;j<npc.length;j++) {
            if (npc[i].rTwo[j] != undefined) {
                ...
                if (npc[i].rTwo[j].attraction > 10 && (counter%10==0)) {npc[i].rTwo[j].attraction --};
...
function killCharacter(char) {
    ...
    for (var i=0;i<char.rTwo.length;i++) {
        if (char.rTwo[i] != undefined) {
            ...
            if (char.rTwo[i].commitment == 1 && npc[i].rTwo[npc.indexOf(char)] != undefined) {
                npc[i].rTwo[npc.indexOf(char)].commitment = 0;
                npc[i].rTwo[npc.indexOf(char)].happiness -= 10000;
...
function fullCharInfoPanel(char) {
    ...
    for (var h=0;h<raray.length;h++) {
        if (npc[raray[h].originalIndex].alive == 1) {
            var rela = "";
            var lvr = "";
            ...
            ra += ("<tr><th scope='row'>" + (h+1) + "</th><td>" + raray[h].rName + lvr + rela + "</td><td>" + npc[raray[h].originalIndex].ageInYears() + "</td><td>" + raray[h].friendship + "</td><td>" + raray[h].attraction + "</td><td>" + raray[h].romance + "</td><td>" + raray[h].interactions + "</td><td>" + raray[h].coitus + "</td></tr>")
        }
    }
    var aa = "";
    var aaray = [];
    ...
    var pnl = (
        ...
        "<div class='row'><div class='col'>" +
        '<p>Relationships:</p><table class="table table-sm"><thead><tr><th scope="col"></th><td>Name</td><td>Age</td><td>Friendship</td><td>Attraction</td><td>Romance</td><td>Interactions</td><td>Coitus</td> </tr> </thead><tbody>' +
        ra + "</tbody></table>" +
        "</div></div>"  // close out the row
        // "<div class='row'><div class='col'>" +
        // "Favorite Activities:<br><ul>" +
...
function updateDash() {
    ...
    for (var l=0;l<textLog.length;l++) {
        if (textLog[l][0] == counter) {
            $("#textlog").append(textLogPrefix() + textLog[l][1] + "<br>");
        }
    }
...
function killRandom() {
    var kTarget;
    do {
        kTarget = pickFrom(npc)
    } while (kTarget.alive==0);
    killCharacter(kTarget);
}
...
$("#testbutton3").click(function() {addNPC();});
$("#murderbutton").click(function() {killRandom();});
...
function petLoop() {
    ...
    if ((counter % 100) == 0 && pet.alive == 1) {
        $("#petpanelstatus").html("Pet is resting.");
    }
    var now = new Date();

        ...
        <div class="row">
            <div class="col">
                ...
                <p>
                    ...
                    <div id="dashpanel">
                        <p>Year <span id="years">1</span> | Day <span id="counter"></span> | Population: <span id="population"></span></p>
                    </div>
                </p>
            </div>
...
function whupEee(persona, personb) {
    if (persona.rTwo[npc.indexOf(personb)].attraction > 100 && persona.ageInYears() >= 5) {
        if (randomInt(1,3) == 1) {
            ...
            if (personb.rTwo[npc.indexOf(persona)].attraction > 50) {
                ...
                if (w == 0 && randomInt(1,5) == 1) {
                    ...
                    if (persona.gender == 0 && persona.pregnant == 0 && persona.ageInYears() >= puberty && randomInt(1,7) == 1) {
                        persona.pregnant = 1;
...
function resolveDeaths() {
    for (var i=0;i<npc.length;i++) {
        if (npc[i].ageInYears() > 75 && randomInt(1, 20) == 1) {
            npc[i].alive = 0;
            $("#textlog").append("On day " + counter + ", " + npc[i].firstName + " died. RIP.<br>");
        }
    }
}
...
function describeRelationship(persona, personb) {
    ...
    } else if (persona.rTwo[npc.indexOf(personb)].romance > 10) {
    ...
    if (persona.relatives[npc.indexOf(personb)] != undefined) {
        description += (personb.firstName + " is " + persona.firstName + "'s " + persona.relatives[npc.indexOf(personb)] + ". ")
    }
    if (persona.rTwo[npc.indexOf(personb)].coitus > 0) {
...
function relaParser(persona, personb) {
    if (persona.relatives[npc.indexOf(personb)] != undefined) {
        if (persona.relatives[npc.indexOf(personb)] == "parent" && personb.gender == 0) {
            return "mother";
        }
    }
    console.log("tbd");
...
function createCharPanel(id) {
    var panel = (
        "<hr><div class='row' id='npc"+id+"'><div class='col-8'>Name: <span id='npcname" + id + "'><strong></strong></span><br>" +
            "Age in years: <span id='npcyears" + id + "'></span><br>" +

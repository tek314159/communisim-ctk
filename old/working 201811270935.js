...
var specialEvents = [
    ...
    },
    {
        ...
        "effect": function() {
            ...
            stranger.ageInDays = (randomInt((25*365), (55*365)));
    ...
    },
    {
        ...
        "effect": function() {
            var origin = pickFrom(localeList);
            addNPC();
            var stranger = npc[npc.length-1];
            stranger.ageInDays = (randomInt((15*365), (65*365)));
            stranger.surName = pickFrom(origin.families);
            $("#npc"+stranger.oIndex).attr("title",stranger.fullName());
            $("#npcname"+stranger.oIndex).text(stranger.fullName());
            addHistory((`the community welcomed ${stranger.fullName()}, an <strong>immigrant</strong> from <strong>${origin.name}</strong>.`));
        },
        "flavortext":"Something happens!!"
    },
    {
...
function lfSaveGame() {
    ...
    localforage.setItem('disPointThreshold', disPointThreshold)}).then(function() {console.log(`dispoithre passed`);}).then(function(){
    ...
    localforage.setItem('teamLeader', teamLeader)}).then(function() {$(".gamecontrolbuttons").attr("disabled",false);updateGameControlStatus('Game saved.');console.log(`teamlead passed`);})

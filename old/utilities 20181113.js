...
function createCharPanel(id) {
    var panel = (
        "<div class='row charcard' id='npc"+id+"'><div class='col-8'>Name: <a href='javascript:;' data-npcid='"+id+"' onclick='modalFunction(" + id + ")' class='charname' data-toggle='modal' data-target='#exampleModal'><span id='npcname" + id + "'>" + npc[id].fullName() + "<strong><a href='javascript:;'></strong></span></a> (<span id='npcgender" + id + "'>" + upCase(npc[id].genderFunc()) + "</span>)<br>" +
    ...
    for (var i=0;i<char.rTwo.length;i++) {
        if (char.rTwo[i] != undefined) {
            ...
            if (char.rTwo[i].commitment == 1 && npc[i].rTwo[npc.indexOf(char)] != undefined) {
                ...
                npc[i].rTwo[npc.indexOf(char)].happiness -= 10000; // why isn't this making an error?
                npc[i].happiness -= 10000;
                //console.log("breaking up the dead");
            }
            if (npc[i].relatives[npc.indexOf(char)] == 1 && npc[i].rTwo[npc.indexOf(char)] != undefined) {
                npc[i].happiness -= 1000;
            }
        }
    }
    //displayCharPanels();
    //console.log(typeof char.oIndex);
    //console.log("removing char.oIndex " + liveNPC.indexOf(char.oIndex));
    //testyuu = [1,2,3];
    //testyuu.splice(0,1);
    liveNPCo = liveNPC.splice(liveNPC.indexOf(char.oIndex),1);
    //console.log(liveNPC);
    // for (var j=0;j<liveNPC.length;j++) {if (npc[liveNPC[j]].alive == 1) {console.log("liveNPC is still alive")} else {console.log("liveNPC is actually dead")}}
...
function calcLoopTime() {
    ...
    console.log("100 days happens in " + secdif + " seconds");
}

...
var pregBaseOdds = 100; // base odds of getting pregnant (at puberty) are 1 in pregBaseOdds
        ...
        if (personb.rTwo[persona.oIndex].attraction > kissBackReq) {
            ...
            if (randomInt(1,5) == 1) {
                if (persona.rTwo[personb.oIndex].coitus == 0) {
                    ...
                    if (persona.relatives[personb.oIndex] != undefined) {suf = (persona.firstName + " (age " + persona.ageInYears() + ") slept with " + personb.firstName + " (age " + personb.ageInYears() + "), " + persona.pPronoun() + " " + relaParser(persona, personb) +  ", for the first time. They committed <strong>incest</strong>.")}
    ...
    for (var i=0;i<liveNPC.length;i++) {
        ...
        for (var j=0;j<llen;j++) {
                ...
                if (r != undefined) {
                    ...
                    var aexp = 1;
                    /* if (r.friendship > 0) {
                        while (2**fexp < Math.abs(r.friendship)) {
                            adder += 2 ** fexp;
                            fexp ++;
                        };
                    } else if (r.friendship < 0) {
                        while (2**fexp < Math.abs(r.friendship)) {
                            adder -= 2 ** fexp;
                            fexp ++;
                        };
                    ...
                    if (r.attraction > 0) {
                        while (2**aexp < Math.abs(r.attraction)) {
                            adder += 2 ** aexp;
                            aexp ++;
                        };
                    } else if (r.attraction < 0) {
                        while (2**aexp < Math.abs(r.attraction)) {
                            adder -= 2 ** aexp;
                            aexp ++;
                        };
                    ...
                    /* if (r.attraction > 0) {
                        while (2**fexp < Math.abs(r.attraction)) {
                            adder += 2 ** aexp;
                            fexp ++;
                        };
                    } else if (r.attraction < 0) {
                    ...
                    if (atc == 0 && char.relatives[j] != undefined) {adder = Math.floor(adder/2);}
                } 
                if (adder > char.social) {
                    p.push(adder);
                } else {
                    p.push(char.social);
                }
...
function updateDash() {
    ...
    for (var m=0;m<(textLog.length-textLogPointer);m++) {
        if (textLog[textLogPointer + m][0] == counter) {
            $("#textlog").prepend("<span class='textlogentry'>" + textLogPrefix() + textLog[textLogPointer + m][1] + "</span><br>");

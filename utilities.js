...
function weightedChooserZero(array1, array2) {
    var weightedArray = [];
    var arrayItemCounter = 0;
    var arlen = array1.length;
    for (i=0;i<arlen;i++) {
        if (array2[i] > 0) {
            weightedArray.push([array1[i], array2[i]]);
            arrayItemCounter += array2[i];
        } else {
            weightedArray.push([array1[i], 0]);
            arrayItemCounter += 0;
        }
    }
    var c = randomInt(1, arrayItemCounter);
    var ct = 0;
    var choice = 0;
    while (c > 0) {
        c -= weightedArray[ct][1];
        choice = ct;
        ct ++;
    }
    return array1[choice];

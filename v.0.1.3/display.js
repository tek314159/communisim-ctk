...
function fullCharInfoPanel(char) {
    ...
    var pnl = (
        ...
        "<div class='row'><div class='col'>" +
        '<div class="table-responsive"><table class="table table-sm table-responsive"><thead><tr><th scope="col"></th><td>Relationships</td><td></td><td>Age</td><td>Friendship</td><td>Attraction</td><td>Romance</td><td>Interactions</td><td>Coitus</td> </tr> </thead><tbody>' +
        ra + "</tbody></table></div>" +
        "</div></div>"
    )
    return pnl;
...
function relCharInfoPanel(char) {
    ...
    var pnl = (
        ...
        '<div class="table-responsive"><table class="table table-sm"><thead><tr><th scope="col"></th><td>Relationships</td><td></td><td>Age</td><td>Friendship</td><td>Attraction</td><td>Romance</td><td>Interactions</td><td>Coitus</td> </tr> </thead><tbody>' +
        ra + "</tbody></table></div>" +
        "</div></div>"
    )
    return pnl;
...
function createLocalePanel(id) {
    var panel = (
        ...
        <div class='card loccard mt-3' id='loc${id}' title="${localeList[id].name}" data-distance="${localeList[id].distance()}">
            <div class='row m-1'>
                <div class='col'>
                    Name: <a href='javascript:;' class="localecard" data-locid='${id}'
                    data-toggle="popover" data-container="body" data-offset="5" data-html="true" title="${localeList[id].name}" data-content="Nothing to report." data-trigger="hover" 
                    onclick='modalFunction("${id}")' class='locname' id='locname${id}'>${localeList[id].name}</a><br>
                    Population: <span id="locpop${id}">${localeList[id].popCalc()}</span><br>
            ...
            <div class='row m-1'>
                <div class='col'>
                    Surnames: <span id="locfamilies${id}">${localeList[id].stringFamilyList()}</span><br>
                </div>
            </div>
        </div>
        `
...
function localePopover(locid) {
    let returnString = "";
    let famList = localeList[locid].familiesTwo;
    for (let i=0;i<famList.length;i++) {
        returnString += `The ${famList[i][0]} Family: ${famList[i][1]} people<br>`;
    }
    return returnString;
}
...
function refreshUI(section) {
    if (section == "leaderpanel") {

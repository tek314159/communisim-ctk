...
function removeCharacter(char, meth) {
    ...
    if (meth == 'kill') {
        ...
        addHistory((char.fullName() + " <strong>died</strong> at age " + char.ageInYears() + ". The community mourns."));
        stats.totalDeaths ++;
    }
...
function checkIndices() {for (var i=0;i<npc.length;i++) if (npc[i] != undefined && npc[i].oIndex != i) {console.log("indices do not match");paused=1}}
